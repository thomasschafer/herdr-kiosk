...
mod tests {
    ...
    fn picker_pin_and_open_filter_defaults_are_collision_free() {
        ...
        for mode in [BindingMode::Repo, BindingMode::Branch] {
            assert_eq!(
                keys.command_for(mode, "C-t".parse().unwrap()),
        ...
        let defaults = default_keys_toml();
        for taken in [
            "ctrl+c", "ctrl+h", "ctrl+x", "ctrl+o", "ctrl+n", "ctrl+p", "ctrl+w", "ctrl+r",
        ] {
            assert_ne!(taken, "ctrl+t");
            assert_ne!(taken, "ctrl+f");
        }
        assert!(defaults.contains("\"ctrl+t\" = \"toggle_pin\""));
        assert!(defaults.contains("\"ctrl+f\" = \"toggle_open_filter\""));
        assert!(!defaults.contains("\"ctrl+b\" = \"toggle_pin\""));
    }
